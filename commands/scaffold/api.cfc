...
component {
	...
	**/
	function run(
		...
		// entity defaults
		var entityName = lcase(listLast( arguments.entity, "." ));
		...
		if( arrayLen( md.properties ) ){
			...
			print.greenline(serializeJson(md.properties))

			//********************** generate handler ************************************//

			// Read Handler Content
			var hContent = fileRead( '/scaffold/templates/api/HandlerContent.txt' );
			// Token replacement
			hContent = replacenocase( hContent, "|entity|", entityName, "all" );
			hContent = replacenocase( hContent, "|entityPlural|", lcase( arguments.pluralName ), "all" );
			hContent = replacenocase( hContent, "|moduleName|", lcase( arguments.moduleName ), "all" );
			...
			//********************** generate service ************************************//
			savecontent variable="local.sData"{
				include '/scaffold/templates/api/ServiceContent.cfm';
			}

			// Read Service Content
			var sContent = fileRead( '/scaffold/templates/api/ServiceContent.txt' );
			// Token replacement
			sData = replaceNoCase( sData, "%cf", "#chr(60)#cf", "all" );
			sData = replaceNoCase( sData, "%/cf", "#chr(60)#/cf", "all" );
			sContent = replacenocase( sContent, "|entity|", ucfirst( entityName ), "all" );
			sContent = replacenocase( sContent, "|entityPlural|", lcase( arguments.pluralName ), "all" );
			sContent = replacenocase( sContent, "|moduleName|", lcase( arguments.moduleName ), "all" );
			sContent = replacenocase( sContent, "|pk|", metadata.pk, "all" );
			sContent = replacenocase( sContent, "|mockData|", sData, "all" );

			// Write Out Service
			var hpath = '#arguments.modelsDirectory#/#entityName#Service.cfc';
			// Create dir if it doesn't exist
			directorycreate( getDirectoryFromPath( hpath ), true, true );
			file action='write' file='#hpath#' mode ='777' output='#sContent#';
			print.greenLine( 'Generated Service: #hPath#' );
